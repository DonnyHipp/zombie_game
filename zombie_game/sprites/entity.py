...
class Entity(arcade.Sprite):
    ...
    def load_texture_set(self, i):
        res = (
            [arcade.load_texture(f"{self.main_path}_lstep_{i}.png")]
            * self.update_per_frame
            + [arcade.load_texture(f"{self.main_path}_idle_{i}.png")]
            * self.update_per_frame
            + [arcade.load_texture(f"{self.main_path}_rstep_{i}.png")]
            * self.update_per_frame
            + [arcade.load_texture(f"{self.main_path}_idle_{i}.png")]
            * self.update_per_frame
        )
    ...
    def update_animation(self, delta_time):
        ...
        if self.change_y < 0 and self.character_face_direction != self.back_direction:
            ...
            self.change_y > 0 and self.character_face_direction != self.front_direction
        ):
            if self.change_x == 0:
                self.character_face_direction = self.front_direction
            ...
            self.change_x > 0 and self.character_face_direction != self.right_direction
        ):
