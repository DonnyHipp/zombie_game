...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        self.tile_map = arcade.tilemap.TileMap(
            map_file=tmx_map, scaling=1, layer_options=layer_options
        )
        ...
        self.spawn_points = self.scene.get_sprite_list(
            settings.WAVE_LIST[self.current_wave]["spawn"]
        )
        self._zombie_spawn()
    ...
    def on_update(self, delta_time: float):
        ...
        self.shoot_logic()
        self.zombie_logic(delta_time)
        self.death_zone_logic(delta_time)
        self.player_conditions_logic(delta_time)
        self.spawn_logic(delta_time)
        self.center_camera_to_player()
        self.scene.update()
        self.scene.update_animation(delta_time, ["Player", "Zombie"])
    ...
    def zombie_logic(self, delta_time):
        ...
        check_zom = arcade.check_for_collision_with_list(
            self.player, self.scene["Zombie"]
        )
    ...
    def spawn_logic(self, delta_time):
    ...
    def player_conditions_logic(self, delta_time):
        # переключение волн и спавн
        if self.enemy_amount == 0:
            if self.current_wave != 0:
                new_wave = self.current_wave + 1
    ...
    def death_zone_logic(self, delta_time):
        # смертельная зоны чек
        check = arcade.check_for_collision_with_list(self.player, self.scene["death"])
    ...
    def _zombie_spawn(self, first=None):
        ...
        спавн зомби
        """
        start_zombie_point = self.spawn_points[
            round(random.randint(0, len(self.spawn_points) - 1))
        ]
        zombie = Zombie(
            start_zombie_point.center_x, start_zombie_point.center_y, 1.3, "weak"
        )
    ...
    def _check_bullet_coll(self, bullet):
        hit_list = arcade.check_for_collision_with_lists(
            bullet,
            [self.scene["Walls"], self.scene["Zombie"]],
        )
